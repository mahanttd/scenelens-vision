import type { BoundingBox, Detection, HoldingEstimate } from "./types";
...
function positionPhrase(box: BoundingBox) {
  ...
  const horizontal =
    centerX < 0.34 ? "left" : centerX > 0.66 ? "right" : "center";
  const vertical =
    centerY < 0.34 ? "upper" : centerY > 0.66 ? "lower" : "middle";
...
function inferSetting(labels: Set<string>) {
  if (["keyboard", "laptop", "mouse", "monitor"].some((label) => labels.has(label))) {
    return "workspace";
  }
  if (["oven", "microwave", "refrigerator", "sink", "toaster"].some((label) => labels.has(label))) {
    return "kitchen-like area";
  }
  if (["car", "bus", "truck", "traffic light"].some((label) => labels.has(label))) {
    return "road or parking area";
  }
  if (["couch", "television", "tv"].some((label) => labels.has(label))) {
    return "living area";
  }
  if (["bed"].some((label) => labels.has(label))) return "bedroom-like area";
  if (["toilet", "toothbrush"].some((label) => labels.has(label))) {
    return "bathroom-like area";
  }
  return null;
}
...
export function describeScene(detections: Detection[]) {
  if (detections.length === 0) {
    return "I can't confidently identify any objects in the current view yet.";
  }
  ...
  const labels = new Set(groups.keys());
  const setting = inferSetting(labels);
  const ranked = [...groups.entries()].sort(([, a], [, b]) => {
    const aScore = Math.max(...a.map((item) => item.confidence)) + a.length * 0.04;
    const bScore = Math.max(...b.map((item) => item.confidence)) + b.length * 0.04;
    return bScore - aScore;
  });
  const details = ranked.slice(0, 6).map(([label, items]) => {
    const friendly = friendlyLabel(label);
    const count = items.length;
    const strongest = [...items].sort((a, b) => b.confidence - a.confidence)[0];
    return `${count === 1 ? articleFor(friendly) : count} ${pluralize(friendly, count)} ${positionPhrase(strongest.box)}`;
  });
  const opening = setting
    ? `This looks like a ${setting}.`
    : "In the current view,";
  return `${opening} I can see ${formatList(details)}.`;
}
...
export function describeHolding(detections: Detection[]) {
  ...
  const label = friendlyLabel(estimate.object.label);
  return `A person may be holding ${articleFor(label)} ${label} — ${Math.round(estimate.confidence * 100)}% spatial confidence.`;
}
...
export function describeTable(detections: Detection[]) {
  ...
  if (tables.length === 0) {
    const lowerObjects = detections
      ...
      .map((detection) => friendlyLabel(detection.label));
  ...
  return objects.length
    ...
    : "A table is visible, but I can't identify an object on it with enough confidence.";
}
...
export function answerLocalQuestion(question: string, detections: Detection[]) {
  ...
  if (/describe|summary|scene|what.*(?:see|visible)/.test(normalized)) {
  ...
  const mentioned = labels.find(
    (label) =>
      normalized.includes(label) ||
      normalized.includes(friendlyLabel(label).toLowerCase()),
  );
  if (mentioned) {
    const matches = detections.filter((detection) => detection.label === mentioned);
    const label = friendlyLabel(mentioned);
    return `I can see ${matches.length} ${pluralize(label, matches.length)}; the strongest detection is ${Math.round(Math.max(...matches.map((item) => item.confidence)) * 100)}% confident.`;
  }
  return `${describeScene(detections)} Try asking about a specific visible object, a person count, or what's on a surface.`;
}
