...
The browser loads `public/models/yolov8n.onnx` once and uses ONNX Runtime Web’s WebAssembly execution provider. Each video frame is resized and letterboxed to 640×640, converted to a normalized channel-first tensor, and processed locally. SceneLens reads the 80 COCO class scores, filters detections, maps coordinates back to the displayed source, and applies class-aware non-maximum suppression. Live inference is throttled to roughly one pass every 700 ms and never sends continuous frames to a server.
...
Camera access requires HTTPS on a deployed domain. Keep the 12.8 MB ONNX model and ONNX Runtime assets cacheable for faster repeat visits.
